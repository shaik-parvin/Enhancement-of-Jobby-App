...
class AllJobs extends Component {
  ...
  onGetJobsView = () => {
    ...
    if (locationInputs.length === 0) {
      FinaljobsData = jobsData
    } else {
      const filteredData = jobsData.filter(each =>
        locationInputs.includes(each.location),
      )
      FinaljobsData = filteredData
    }
  ...
  onGetLocationInputOption = event => {
    const {locationInputs} = this.state
    const inputNotInList = locationInputs.filter(
      eachItem => eachItem === event.target.id,
    )
    if (inputNotInList.length === 0) {
      this.setState(
        prevState => ({
          locationInputs: [...prevState.locationInputs, event.target.id],
        }),
        this.onGetJobDetails,
      )
    } else {
      const filteredData = locationInputs.filter(
        eachItem => eachItem !== event.target.id,
      )
      this.setState(
        // eslint-disable-next-line no-unused-vars
        prevState => ({locationInputs: filteredData}),
        this.onGetJobDetails,
      )
    }
  }
      ...
      {locationList.map(eachItem => (
        <li className="li-container" key={eachItem.locationId}>
          <input
            ...
            type="checkbox"
            onChange={this.onGetLocationInputOption}
          />
